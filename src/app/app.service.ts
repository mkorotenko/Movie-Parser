...
const getRatings = function (node) {
  node.childNodes.forEach(n => {
    const ratingNode = findAttr(n, 'current-rating');
    if (ratingNode) {
...
export class AppService {
  ...
  //public rootContent = this.client.get('api/acc/content?page=2').pipe(
    ...
    map((res: object[]) => res.map(element => {
      ...
      const ratingNode = findAttr(element, 'current-rating');
      if (ratingNode) {
      ...
      const hrefNode = findAttr(element, 'zagolovki');
      if (hrefNode) {
      ...
      const details = findAttr(element, 'shortimg');
